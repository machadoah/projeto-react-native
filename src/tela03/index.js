...
const VisualizarReceitasPage = () => {
  ...
  const handleVerMais = (receita) => {
    //navigation.navigate('DetalhesSugestao', { sugestao: receita });
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Sugestões do chefe 🧑🏽‍🍳</Text>
      <FlatList
        data={receitas}
...
const ReceitasPage = () => {
  ...
  const renderReceita = ({ item }) => (
    <View>
      ...
      <Image source={{ uri: item.link_imagem }} style={{ width: 200, height: 200 }} />
  ...
  return (
    <View>
      <VisualizarReceitasPage /> {/* Adiciona o componente da primeira página aqui */}
